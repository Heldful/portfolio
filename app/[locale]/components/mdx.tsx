...
const components = {
  ...
  Image,
  Game: async ({ className, ...props }) => {
    const t = await getTranslations("Projects");
    return (
      <div
        className={clsx(
          "w-full h-full flex flex-wrap justify-center items-center relative"
        )}
      >
        <p className={clsx("w-full lg:hidden -mt-2 mb-8 text-red-400")}>
          {t("game.deviceSizeWarning")}
        </p>
        <div
          className={clsx(
            "relative rounded-lg border overflow-hidden",
            // 16:9 ratio
            "min-w-[360px] min-h-[202px]",
            "md:min-w-[640px] md:min-h-[360px]",
            "lg:min-w-[960px] lg:min-h-[640px]",
            className
          )}
        >
          <iframe
...
export function Mdx({ code, locale }: MdxProps) {
  const Component = useMDXComponent(code, { locale });
